...
export const intro: ReactElement = (<p>Semantic Kernel, Microsoft'un C#, Python veya Java'da LLM'leri, eklentileri ve planlayıcıları düzenlemek için açık kaynaklı SDK'sıdır.</p>);
...
export const body: ReactElement = (
  <>
    ...
    <h2>Ne olduğu</h2>
    <p>Semantic Kernel, AI hizmetlerini (sohbet, gömme, görüntü), eklentileri (modelin çağırabileceği çağrılabilir işlevler), belleği (vektör depoları) ve bir hedefi bir dizi işlev çağrısına dönüştüren planlayıcıları birbirine bağlayan bir Kernel nesnesi sunar. Diller arasında neredeyse eşitlik sağlayan NuGet, PyPI ve Maven paketleri olarak mevcuttur.</p>
...
print(reply)`}</pre>
    ...
    <ul>
      ...
      <li>2. Karmaşık hedefleri sıralı eklenti çağrılarına ayırmak için Handlebars veya Stepwise planlayıcıyı kullanın.</li>
      <li>3. Getirme artırımlı sohbet için bir bellek deposu (Azure AI Search, Qdrant, Redis) ekleyin.</li>
    </ul>
    <h2>Tuzaklar ve ipuçları</h2>
    <p>Semantic Kernel büyük ölçüde bağımlılık enjeksiyonuna dayanır; özellikle .NET'te, bir Kernel'i manuel olarak oluşturmak yerine hizmetleri ana bilgisayar oluşturucuya kaydedin &mdash; böylece uygun günlük kaydı ve yapılandırma elde edersiniz. Uzun eklenti zincirlerinde hata ayıklayabilmek için OpenTelemetry entegrasyonunu erken kullanın.</p>
    <p>Planlayıcılar token'ları hızla tüketebilir; iş akışı bilindiğinde açık işlev kompozisyonunu tercih edin ve planlayıcıları açık uçlu hedefler için saklayın. Python ve .NET SDK'ları ara sıra farklılaşır &mdash; üretimde sürümleri sabitleyin ve önizleme paketlerindeki son değişiklikler için sürüm notlarını kontrol edin.</p>
